...
<img src="p8105_hw6_at2710_files/figure-markdown_github/unnamed-chunk-1-1.png" width="90%" /> Comment: The odds of solving crimes for non-white victims are lower in every city except Baton Rough, LA and Tampa, FL. However, the odds for those two cities are not statistically higher. On the other hand, a majority of cities has lower odds for solving cases for non-white vicitims with statistical significance such as Birmingham, AL, Baltimore, MD, Buffalo, NY and Tulsa, OK. Among the cities with dismal odds for non-white victims, Boston, MA seems to have worst odds for solving crimes if the victims are non-white.
...
First I created a new smoking variable (binary) from `smoken` since the data is zero heavy and wouldn't make much sense to try to establish a linear relationship between `smoken` and the outcome. Next I checked the missingness in the dataset and it appear to contain no missing information.
...
First I split the dataset into traing and test set with 80:20 allocation. Then using the training set, I came up with the optimal lambda (tuning parameter) which minimize the MSE using he cross-validation method. The best lamda derived is 1.3457781. and the model estimates are:
...
<img src="p8105_hw6_at2710_files/figure-markdown_github/unnamed-chunk-3-1.png" width="90%" />
